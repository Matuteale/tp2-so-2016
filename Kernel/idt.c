...
void syscall_handler(uint64_t arg_3, uint64_t arg_2, uint64_t arg_1, uint64_t syscall)
{
  ...
  if(syscall == 0x1){ncPrintDec((int)arg_3);}
    ...
    case 0x2: kill_process((int) arg_1); break;
		case 0x3: if(process->foreground == 1 || process->state == READY){sys_readKeyboard((char *)arg_2);} break;
		case 0x4: if(process->foreground == 1 || process->state == READY){sys_displayWrite((char *)arg_2, arg_1);} break;
		...
		case 0x8: if(process->foreground == 1 || process->state == READY){clearscreen();} break;
